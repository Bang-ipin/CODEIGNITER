<script src="<?=base_url('assets/admin/pages/scripts/komponen.js');?>" type="text/javascript"></script>
		...
		<script>
			...
			tinymce.init({
				...
				$('#subatributview').change(function(event){   
					var id = $(this).val()
					$.ajax({
						url:'<?=site_url('shop_produk/products/subatribut_view')?>',
						data:"id="+id,
						type:"post",
						dataType: "json",
						success: function(data) {
							$('#subattr').html(data.html);
						}
					});
				});

				$('#subkategoriview').change(function(event){   
						var kategori_id = $(this).val()
						$.ajax({
						url:'<?=site_url('shop_produk/products/subcategory_view')?>',
						data:"kode_kategori="+kategori_id,
						type:"post",
						dataType: "json",
						success: function(data) {
