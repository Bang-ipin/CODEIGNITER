...
<div class="alert alert-success">Data Barang Masuk</div>
	<form action="<?php echo base_url('toko_ringroad/pembelian/cart');?>" method="POST">	
...
$(function () {
		...
		if(confirm("Apakah anda ingin menghapus data pembelian ini?")){
          $.ajax({
            type: 'get',
            url: '<?= base_url('toko_ringroad/pembelian/delete_cart');?>',
            data: $('form').serialize(),
	  ...
	  function hapusitem(id) {
		if(confirm("Apakah anda ingin menghapus data pembelian ini?")){
          $.ajax({
            type: 'POST',
            url: '<?= base_url('toko_ringroad/pembelian/deleteitemcart');?>',
            data: 'id='+id,
			error: function (xhr, ajaxOptions, thrownError) {
				return false;		  	
			},
