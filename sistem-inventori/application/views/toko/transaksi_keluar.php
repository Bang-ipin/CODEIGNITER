	<script type="text/javascript">
		...
		var ajaxku;
			function ambildata(id_pelanggan){
				ajaxku = buatajax();
				var url="<?php echo base_url('toko_ringroad/penjualan/ajaxpelanggan')?>";
				url=url+"?q="+id_pelanggan;
		...
		if (ajaxku.readyState==4){
			...
			if (data.length>0){
				document.getElementById("pelanggan").value=data;
				}else{
		...
		function getcart() {
		$.get('<?= base_url('toko_ringroad/penjualan/lihat');?>', function(data) {
		  ...
		  $(function () {
			...
			if(Number(jumlah) > Number(stock) || jumlah=="0")
			{
			  ...
			  $.ajax({
				type: 'get',
				url: '<?= base_url('toko_ringroad/penjualan/cart');?>',
				data: $('form').serialize(),
						...
						<div class="col-xs-12">
							<div class="box">
								<div class="box-body">
									...
									<form action="<?=base_url('toko_ringroad/penjualan/checkout')?>" method="post" onsubmit="return validasi(this)">
										<div class="form-horizontal">
											...
											<div class="form-group">
												<div class="col-xs-3">
													<label for="pelanggan">Distributor</label>
												</div>
												<div class="col-xs-6">
													<?php 
													$pelanggan_selected= $this->input->post('pelanggan')?$this->input->post('pelanggan'):'';
													echo form_dropdown('pelanggan',$dd_pelanggan,$pelanggan_selected,$attribute);
													?>
												</div>
											</div>
											<hr>
															...
															<td>
																<select name="kode_barang" class="form-control select2"  id="kode_barang" onchange="changeValue(this.value)" required >
																	...
																	<?php
																		...
																		$query = $this->db->get('stok_toko2 a');  
											...
											<div>
												...
												<a href="<?=base_url('toko_ringroad/penjualan')?>" class="btn btn-danger">Cancel</a>
											</div>
										</div>
									</form>
								</div>
		...
		<script type="text/javascript">
			function validasi(form){
				...
				if (form.pelanggan.value==null || form.pelanggan.value==""){
