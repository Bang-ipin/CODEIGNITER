...
<div>&nbsp </div>
	<?php if(isset($tgl_awal))
		{
			echo"<div class='alert alert-info'>Data transaksi penjualan Toko Ringroad dari tanggal ".$tgl_awal." sampai dengan tanggal ".$tgl_akhir." adalah sebagai berikut:<button type='button' class='close' data-dismiss='alert'>&times;</button></div>
			...
			echo "	<div class='alert alert-info'>Data semua transaksi penjualan Toko Ringroad sebagai berikut: <button type='button' class='close' data-dismiss='alert'>&times;</button></div>
			...
			$no=0;
				if(!empty($query)){
					...
					<tr>
						...
						<td align="center">Rp. <?php echo number_format($rows->harga_jual,2,',','.');?></td>
						<td align="center">Rp. <?php echo number_format($rows->total,2,',','.');?></td>
					</tr>
					<?php
					 }
					echo "<tr>
							<td colspan='8' style='text-align: center; background: #49afcd'><strong>Total Penjualan</strong></td>
...
<script type="text/javascript">
	...
	$(function(){
        $("#cetakpdf").click(function(e) {
			...
			if(	$tgl_awal == '' || $tgl_akhir == '')
			{
				...
				{
				window.open('<?php echo base_url()?>toko_ringroad/lap_penjualan/cetakPdf/'+$tgl_awal+'/'+$tgl_akhir);
			}
        });
    });
	$(function(){
        $("#cetakexcel").click(function(e) {
			...
			if(	$tgl_awal == '' || $tgl_akhir == '')
			{
				...
				{
				window.open('<?php echo base_url()?>toko_ringroad/lap_penjualan/cetakexcel/'+$tgl_awal+'/'+$tgl_akhir);
			}
        });
    });
	...
	$(function(){
        $("#printpdf").click(function(e) {
				window.open('<?php echo base_url()?>toko_ringroad/lap_penjualan/printpdf');
			});
        });
		
	$(function(){
        $("#printexcel").click(function(e) {
            	window.open('<?php echo base_url()?>toko_ringroad/lap_penjualan/printexcel');
			});
        });
